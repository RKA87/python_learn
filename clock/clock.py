...
class Clock(object):
    ...
    def set_Clock(self, hours, minutes, seconds):
        ...
        if type(hours) == int and 0 <= hours and hours < 24: #(normally, hours >=0 and hours <24)
            self._hours = hours #(single undersquare is protected which means it can be used in child class)
        else:
            raise TypeError("Hours have to be integers between 0 and 23!")
        if type(minutes) == int and 0 <= minutes and minutes < 60:
            self.__minutes = minutes #(double undersquare is private and not accessible in other class)
        else:
    ...
    def __str__(self): #02d - precision of 2 digits in first parameter
    ...
    def run(self):
        import time
        """
        This method continuously runs the clock, updating every second.
        """
        while True:
            print(self)
            time.sleep(1)
